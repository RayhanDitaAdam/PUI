...
let DuoCard = function () {
    ...
    const title = c.aboutCompanyTitle || 'Tentang Perusahaan'
    const desc = c.aboutCompanyDesc || 'PT Pergadaian Utama Indonesia (PUI) merupakan perusahaan pergadaian nasional yang menyediakan solusi pendanaan cepat dengan sistem gadai, khususnya untuk barang bernilai tinggi atau barang mewah.'
    const listIntro = c.aboutCompanyListIntro || 'PT Pergadaian Utama Indonesia telah:'
    const listItems = c.aboutCompanyList || [
        'Berizin dan diawasi oleh Otoritas Jasa Keuangan',
        'Beroperasi sesuai regulasi OJK tentang industri pergadaian',
    ]
    const outro = c.aboutCompanyOutro || 'Hal ini menjamin bahwa seluruh layanan PUI berjalan secara legal, aman, dan sesuai standar industri keuangan.'
            ...
            <div className="hidden md:flex about-section mt-44 mx-auto relative justify-center items-center px-4 md:px-0 md:pr-4 md:pl-8 lg:px-4">
                <div className="about-content w-full flex justify-center items-center md:mb-7 relative">
                    <div className="about-card absolute left-0 lg:left-[-2rem] xl:left-[2rem] z-10 md:!w-[25.125rem] lg:!w-[39.4375rem] md:!h-[24.3125rem] lg:!h-[26.1875rem]">
                        <h2 className="md:text-2xl lg:text-[2rem]">{title}</h2>
                        <div className="w-[300px] h-[3px] mt-2 mb-6 md:mb-2 lg:mb-6 bg-white"></div>
                        <div className="feature-list text-white">
                            <p className="mb-2 md:text-[0.875rem] lg:text-base">{desc}</p>
                            <p className="md:text-[0.875rem] lg:text-base">{listIntro}</p>
                            <ul className="ml-0.5 list-disc list-inside text-white md:text-[0.875rem] lg:text-base">
                                {listItems.map((item, i) => (
                                    <li key={i}>{item}</li>
                                ))}
                            </ul>
                            <p className="mt-2 md:text-[0.875rem] lg:text-base">{outro}</p>
                        </div>
                    </div>
