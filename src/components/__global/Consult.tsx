...
import { getConsultContent } from "../../data/content";
...
let Consult = function (props?: ConsultProps) {
  const c = getConsultContent();
  const title = props?.title || c.consultTitle || "Ingin gadai barang<br>Anda?"
  const subtitle = props?.subtitle || c.consultSubtitle || "Konsultasikan sekarang via WhatsApp.<br>Terjamin Kerahasiaan & privasi 100%."
  const buttonText = props?.buttonText || c.consultButton || "Konsultasi Valuasi Sekarang"
  const cardTitle = props?.cardTitle || "Layanan Jemput Barang"
  const cardDesc = props?.cardDesc || "Untuk kenyamanan maksimal, PUI menyediakan layanan jemput aset langsung ke lokasi Anda. Layanan ini dirancang bagi Anda yang menginginkan proses praktis tanpa perlu datang ke kantor. Tim kami akan datang dengan prosedur keamanan ketat dan profesionalisme tinggi dalam menangani setiap aset."
  const cardLinkText = props?.cardLinkText || "Booking Jadwal Jemput Barang"

  const wa = settings.whatsapp
  const consultPhone = c.consultPhone || wa
  const consultMessage = c.consultMessage || "Halo PUI, saya ingin konsultasi valuasi aset."
  const consultWaUrl = buildWaUrl(consultPhone, consultMessage)

  const bookingMessage = "Halo PUI, saya ingin booking layanan jemput aset ke lokasi saya."
  const cardLinkHref = props?.cardLinkHref || buildWaUrl(consultPhone, bookingMessage)
      ...
      <div className="pui-wrap mt-32 mb-12 md:mt-10 md:mb-0 overflow-visible lg:!max-w-[1200px] xl:!max-w-[1350px]">
        <div className="mx-[1rem] hero flex flex-col md:flex-row-reverse items-start md:items-end gap-0 md:gap-8 pt-4 pb-0 px-8 md:p-12 md:pb-0 text-left md:text-left isolate">
          ...
          <div className="md:pl-0 mr-8 md:mr-0 lg:mr-0 md:-mb-5 lg:p-5 lg:mt-20 md:p-1 hero-content relative z-[50] md:z-10 -mt-[19.375rem] flex flex-col items-start w-full md:w-[21.25rem] pb-10 md:pb-10 md:bottom-10">
            ...
            <p className="text-[0.65rem] lg:text-base md:text-sm text-white/70 mb-8 md:mb-7 leading-relaxed max-w-[16.25rem] md:max-w-[17.5rem]"
              dangerouslySetInnerHTML={{ __html: subtitle }} />
            <a href={consultWaUrl} target="_blank" rel="noopener noreferrer" className="btn-wa animate-pulse-whatsapp flex items-center justify-center gap-2 bg-[#006658] border border-white/20 text-white w-full md:w-auto px-6 py-3.5 md:px-5 md:py-3 rounded-xl text-base md:text-sm font-semibold relative z-[60] pointer-events-auto">
        ...
        <div className="bg-white relative z-20">
          <div className="mx-[1rem] grid grid-cols-1 gap-3 md:gap-4 lg:-mt-[1.1rem] md:mt-0 !bg-white pt-2 pb-4 md:pt-[2rem] md:pb-[1rem] lg:py-4 rounded-b-xl md:rounded-b-2xl border-x md:border-none border-b md:border-none border-gray-200 relative z-10" style={{ backgroundColor: 'white' }}>
            <div className="card bg-white border border-gray-200 md:border-gray-200 rounded-xl p-4 md:p-7 flex flex-col">
              ...
              <div className="text-xs lg:text-[10px] md:text-sm text-[#333] leading-relaxed mb-4 md:mb-6"
                dangerouslySetInnerHTML={{ __html: cardDesc }} />
              <a href={cardLinkHref} target="_blank" rel="noopener noreferrer" className="card-link mt-auto flex items-center gap-2 text-[#1B4332] font-semibold text-[9px] md:text-xs cursor-pointer animate-link-arrow underline">
                {cardLinkText}
